...
export const FinanceiroSidebar = () => {
      ...
      <div className="p-8 pb-4">
        <div className="flex items-center gap-3 px-2">
          ...
          <div>
            ...
            <p className="text-[10px] uppercase tracking-widest text-emerald-600 font-bold">Gestão Financeira</p>
          </div>
        </div>
      </div>

      <nav className="flex-1 px-4 py-8 flex flex-col gap-2">
        <SidebarItem to="/financeiro" end icon={<Briefcase className="w-5 h-5" />} label="Dashboard Financeiro" />
        <SidebarItem to="/financeiro/receitas" icon={<TrendingUp className="w-5 h-5" />} label="Contas a Receber" />
        ...
        <button 
          onClick={async () => {
            await logActivity({
              action: 'LOGOUT',
              resource_type: 'auth',
              details: 'Usuário encerrou a sessão a partir do menu lateral Financeiro'
            });
            auth.signOut();
            navigate('/login');
          }}
